...
import { HistoryItem, Badge } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ history, unlockedBadges, allBadges, onViewHistoryItem }) => {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                 <div className="bg-white dark:bg-neutral-800 p-6 rounded-lg shadow-lg border border-neutral-200 dark:border-neutral-700">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-bold text-neutral-800 dark:text-neutral-100">Bias Type Frequency</h3>
                    </div>
                    {biasTypeBreakdown.length > 0 ? (
                        <div className="space-y-4">
                            {biasTypeBreakdown.map(item => (
                                <div key={item.name} className="flex items-center group">
                                    <p className="w-1/3 text-sm font-medium text-neutral-600 dark:text-neutral-300 pr-4 text-right">{item.name}</p>
                ...
                <Badges allBadges={allBadges} unlockedBadges={unlockedBadges} />
            </div>
            ...
            <HistoryList history={history} onViewItem={onViewHistoryItem} />
             ...
             <div className="text-center mt-12">
                <button
                    ...
                    className="flex items-center mx-auto px-4 py-2 bg-transparent border-2 border-trust-blue dark:border-ai-teal text-trust-blue dark:text-ai-teal font-semibold rounded-md text-sm hover:bg-trust-blue/10 dark:hover:bg-ai-teal/10 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
