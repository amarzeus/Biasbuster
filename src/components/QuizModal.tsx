...
const QuizModal: React.FC<QuizModalProps> = ({ isOpen, onClose, onQuizComplete }) => {
    ...
    const handleNextQuestion = () => {
        ...
        if (isLastQuestion) {
            const currentScore = score + (selectedAnswer === quizQuestions[currentQuestionIndex].correctAnswer ? 1 : 0) - (selectedAnswer !== null ? 0 : score);
            const final = selectedAnswer === quizQuestions[currentQuestionIndex].correctAnswer ? score + 1 : score
            setFinalScore(final);
            onQuizComplete(final);
            setShowResults(true);
        } else {
                ...
                <div className="p-8 flex-grow overflow-y-auto">
                    { !showResults ? (
                        ...
                        <div className="text-center flex flex-col items-center justify-center h-full">
                            ...
                            <p className="text-6xl font-bold text-trust-blue dark:text-ai-teal my-4">{finalScore} / {quizQuestions.length}</p>
                             <div className="flex space-x-4">
                                ...
                                <button onClick={handleQuizEnd} className="px-6 py-2 bg-trust-blue hover:bg-trust-blue-light dark:bg-ai-teal dark:hover:bg-ai-teal-light text-white font-bold rounded-md transition-colors">
                                    Done
                                </button>
                            </div>
                        </div>
